 ...
 */
cv::Mat DisplayClass::drawObjectLocations(const cv::Mat& frame, const std::vector<std::tuple<int, cv::Rect>>& objectLocations) {
    cv::Mat resultFrame = frame.clone();

    // Placeholder implementation: Add your logic here to draw object information on the frame.
    // Loop through objectLocations and draw information on resultFrame.
